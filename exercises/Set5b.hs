...
treeMax Empty = 0
treeMax (Node v t1 t2) = max v (max(treeMax t1) (treeMax t2))
...
cull val (Node v t1 t2)
  | v == val = Empty
  |otherwise = Node v (cull val t1) (cull val t2)
...
isOrdered (Node v t1 t2) 
  | allValues (<v) t1 && allValues (>v) t2    = isOrdered t1 && isOrdered t2
  |otherwise    = False
...
walk [] (Node v t1 t2) = Just v
walk (StepL:steps) (Node v t1 t2) = walk steps t1
walk (StepR:steps) (Node v t1 t2) = walk steps t2
...
set (StepL:steps) val (Node v t1 t2) = Node v (set steps val t1) t2
set (StepR:steps) val (Node v t1 t2) = Node v t1 (set steps val t2)
...
search x (Node v t1 t2)
  | x == v = Just []
  | search x t1 /= Nothing = Just (StepL : removeMaybe(search x t1))
  | search x t2 /= Nothing = Just (StepR : removeMaybe(search x t2))
  |otherwise = Nothing

  
removeMaybe :: Maybe [Step]  -> [Step]
removeMaybe Nothing = []
removeMaybe (Just steps) = steps

--git commit -a -m "Have not been keeping this repo up to date mut I guess I should push a commit at least rarely, to be at least somewhat safe"