...
sums' prew count i = if count <= i then (prew+count) : sums' (prew+count) (count+1) i else []
...
sorted :: [Int] -> Bool
sorted [] = True  -- I think that this should either return False or error/exception rather than True.
sorted (x:[]) = True
sorted (x:y:xs) 
    | x <= y  = sorted (y:xs)
    | x > y  = False
