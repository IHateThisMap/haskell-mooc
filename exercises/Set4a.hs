...
distinct xs = if (xs == nub xs) then True else False
...
incrementKey key [] = []
incrementKey key (kv:kvs)
    | key == fst kv = (fst kv, snd kv + 1) : incrementKey key kvs
    | otherwise = kv : incrementKey key kvs
...
freqs xs = Map.fromList (foldr freqs' [] (group (sort xs)))

freqs' :: [a] -> [(a, Int)] -> [(a,Int)]
freqs' a b = (head a, length a) : b
...
transfer from to amount bank = 
    case Map.lookup from bank of
        Nothing -> bank  -- the from account doesn't exist,
        Just fBal -> 
            case Map.lookup to bank of 
                Nothing -> bank -- the to account doesn't exist,
                Just tBal -> 
                    if amount < 0 then bank -- the sum is negative
                    else if fBal-amount < 0 then bank -- or the from account doesn't have enough money.
                        else Map.adjust (amount +) to 
                            (Map.adjust (-amount +) from bank) 
...
maxIndex' :: (Ix i, Ord a) => [(i,a)] -> i
maxIndex' ((x,y):[]) = x
maxIndex' ((x,y):(z,w):xys) = if w < y 
    then maxIndex' ((x,y):xys) 
    else maxIndex' ((z,w):xys)