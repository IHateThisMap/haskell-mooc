...
binomial _ 0 = 1
binomial n k = if n == 0 then 0 else (binomial (n-1) k + binomial (n-1) (k-1))
...
countdown x = f x "Ready! "
    where f x s = if x > 0 then f (x-1) (s ++ show x ++ "... ") else s ++ "Liftoff!"
...
smallestDivisor x = f x 2 
    where f x j = if x `mod` j == 0 then j else f x (j + 1)
