...
travel (Velocity mps) (Time s) = Distance (mps*s)
...
data State = Start | Error | Finished | Eggs | Flour | Sugar | FlourAndSugar | Mixed
  deriving (Eq,Show)
...
reverseNonEmpty (x :| []) = (x :| [])
reverseNonEmpty (x :| xs) = makeNotEmpty (reverse xs ++ [x])
  where makeNotEmpty (x:xs) = (x :| xs)
-- toimii myös alemmalla rivillä kommentissa olevalla tavalla jos lisätään importteihin "import Data.List.NonEmpty (reverse)" 
...
passwordAllowed s (MinimumLength ml) = length s >= ml --ml=minlength
passwordAllowed s (ContainsSome (rl:rls)) = if elem rl s then True else passwordAllowed s (ContainsSome (rls)) --rl=requiredLetters
passwordAllowed s (ContainsSome _) = False
passwordAllowed s (DoesNotContain (fl:fls)) = if elem fl s then False else passwordAllowed s (DoesNotContain (fls)) --fl=forbiddenLetters
passwordAllowed s (DoesNotContain _) = True
passwordAllowed s (And c1 c2) = passwordAllowed s c1 && passwordAllowed s c2 --c=condition
passwordAllowed s (Or c1 c2) = passwordAllowed s c1 || passwordAllowed s c2
