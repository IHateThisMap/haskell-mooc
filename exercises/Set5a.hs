...
totalPrice (MkShoppingEntry _ p c) = fromIntegral c * p
...
data Person = PersonData Int String
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = PersonData 90 "Fred"

-- getName returns the name of the person
getName :: Person -> String
getName (PersonData _ name) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (PersonData age _) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (PersonData age _) = (PersonData age name)

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (PersonData _ name) = (PersonData age name)
...
rgb (Mix c1 c2) = zipWith mix' (rgb c1) (rgb c2)
  where mix' c1 c2 = (c1+c2)/2
...
--data OneOrTwo = One { field1 :: FieldType }
--data OneOrTwo = Two { field2 :: FieldType, field3 :: FieldType}

data OneOrTwo t = Two t t | One t 
...
toNat z 
  | z < 0   = Nothing
  | z == 0  = Just Zero
  |otherwise   = Just (_toNat z)
  where _toNat 0 = Zero
        _toNat x = PlusOne(_toNat(x-1))
